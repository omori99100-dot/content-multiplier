...
try:
    ...
    st.error(f"⚠️ خطأ في تحميل التصميم: {e}")
    st.markdown("<style>.stApp{background:#0F172A;}h1,h2,h3,p,div{color:#F8FAFC!important;}</style>", unsafe_allow_html=True)
...
try:
    ...
    st.error(f"⚠️ خطأ في قسم Hero: {e}")
...
try:
    ...
    st.error(f"⚠️ خطأ في قسم Features: {e}")
...
try:
    ...
    st.error(f"⚠️ خطأ في قسم Pricing: {e}")
...
try:
    ...
    st.error(f"⚠️ خطأ في قسم Testimonials: {e}")
...
try:
    ...
    st.error(f"⚠️ خطأ في قسم Footer: {e}")
